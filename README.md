...
0) Build stage-1 binary palm gate model (dev machine)
	...
	- Runtime note: Stage-1 gate now feeds raw RGB by default (no external MobileNetV2 preprocessing) for compatibility with `train_binary_gate.py` exports.
...
- `INFERENCE_DB_PATH`: SQLite path for persistent inference logs (default `reports/inference_log.db`).
...
- `PALM_BINARY_APPLY_PREPROCESS`: set `true` only for legacy binary models that require external MobileNetV2 preprocessing. Keep `false` for current `train_binary_gate.py` exports (default: `false`).

### SQLite Inference Logging
The runtime now writes an append-only SQLite trace for API and CLI inference attempts.

- Default location: `reports/inference_log.db` (override with `INFERENCE_DB_PATH`).
- Tables are auto-created on first use with strict constraints on tags and stage outcomes.
- Each logged request writes exactly 3 stage rows in deterministic order:
	- `binary_gate`
	- `quality_gate`
	- `ripeness_classification`
- DB writes are best-effort and non-fatal: inference/API responses continue even if logging fails.
- Stage compatibility view `v_validation_stage_summary` maps:
	- `binary_gate` + `rejected` -> `binary_gate_reject`
	- `ripeness_classification` + `completed` -> `ripeness_classification`
- Optional pre-deployment schema smoke check:
	- `python scripts/smoke_check_inference_db.py`
	- Custom DB path: `python scripts/smoke_check_inference_db.py --db reports/inference_log.db`
- Full pre-deploy dry run (API + CLI + smoke check + PASS/FAIL report):
	- `python scripts/predeploy_dry_run.py`
	- Example with external PalmDetector dataset:
...
- Quick cleanup of generated dry-run artifacts:
	- `python scripts/cleanup_runtime_artifacts.py`
	- Keep DB but remove generated cache files: `python scripts/cleanup_runtime_artifacts.py --keep-db --all-pyc`
...
- If your runtime starts rejecting most Palm images unexpectedly, verify `PALM_BINARY_APPLY_PREPROCESS=false`.
