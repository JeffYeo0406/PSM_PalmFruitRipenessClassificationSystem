"""Run 7-flow MobileNetV3 reproduction against historical MobileNetV2 experiment flow.
...
best-accuracy MobileNetV3 run for downstream quantization.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run MobileNetV3 7-flow reproduction")
    ...
    parser.add_argument(
        "--log-path",
        default="reports/experiment_log_mobilenetv3_repro.csv",
        help="Separate CSV path for this reproduction run",
    )
    parser.add_argument(
        "--best-summary-path",
        default="reports/mobilenetv3_repro_best_run.json",
        help="Output JSON summary path for selected best run",
    )
    parser.add_argument(
        "--baseline-log",
        default="reports/experiment_log.csv",
    ...
    preprocessing_group.add_argument(
        ...
        action="store_true",
        help="Enable MobileNetV3 built-in preprocessing layer (default).",
    )
    preprocessing_group.add_argument(
        "--no-include-preprocessing-layer",
        dest="include_preprocessing_layer",
        action="store_false",
        help="Disable MobileNetV3 built-in preprocessing layer.",
    )
...
def run_profile(
    ...
    row = {
        ...
        "notes": (
            f"profile={profile.name}; model=MobileNetV3Small; preprocess_family=mobilenet_v3; "
...
def main() -> int:
    ...
    print(f"Running MobileNetV3 reproduction with {len(selected_profiles)} profile(s)")
    ...
    print("\n=== DONE ===")
    print("Best MobileNetV3 run:", best_row)
    if baseline_best:
